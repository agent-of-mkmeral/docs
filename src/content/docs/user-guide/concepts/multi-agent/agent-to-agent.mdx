...
| `client_config` | `ClientConfig` | None | A2A client configuration for authentication and transport settings |
| `a2a_client_factory` | `ClientFactory` | None | **Deprecated.** Use `client_config` instead |
...
When the remote agent's card endpoint or message endpoint requires authentication (SigV4, OAuth, bearer tokens), pass a `ClientConfig` with a pre-configured `httpx_client`:
...
# Configure an httpx client with bearer token authentication
auth_client = httpx.AsyncClient(
    headers={"Authorization": "Bearer <your-token>"},
    timeout=300,
)

# Pass it through client_config — used for both card discovery and message sending
a2a_agent = A2AAgent(
    endpoint="https://my-protected-agent.example.com",
    client_config=ClientConfig(httpx_client=auth_client),
)

result = a2a_agent("Hello, authenticated agent")
print(result.message)
```

For AWS SigV4 authentication, use a library like [`aws-sigv4-httpx`](https://pypi.org/project/aws-sigv4-httpx/) or configure `httpx.Auth`:
...
# Example: custom httpx.Auth subclass for SigV4
auth_client = httpx.AsyncClient(auth=my_sigv4_auth, timeout=300)

a2a_agent = A2AAgent(
    endpoint="https://my-aws-agent.example.com",
    client_config=ClientConfig(httpx_client=auth_client),
)
```
...
:::warning[Timeout responsibility]
When you provide your own `httpx_client` through `client_config`, you are responsible for configuring its timeout. The `A2AAgent.timeout` parameter only applies to internally-created HTTP clients.
:::

:::note[Migrating from `a2a_client_factory`]
The `a2a_client_factory` parameter is deprecated. Replace it with `client_config`:

```python
# Before (deprecated)
from a2a.client import ClientConfig, ClientFactory
factory = ClientFactory(ClientConfig(httpx_client=auth_client))
...
You cannot provide both parameters — doing so raises a `ValueError`.
:::
