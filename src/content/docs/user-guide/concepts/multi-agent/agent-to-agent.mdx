...
When the remote agent requires authentication, pass a `ClientConfig` with a pre-configured `httpx_client`. The same client is used for both card discovery and message sending.
...
# Bearer token, SigV4, OAuth — configure httpx as needed
...
For AWS SigV4, use a custom `httpx.Auth` subclass or a library like [`aws-sigv4-httpx`](https://pypi.org/project/aws-sigv4-httpx/):
...
:::note
When you provide your own `httpx_client`, you are responsible for its timeout configuration. The `a2a_client_factory` parameter is deprecated — use `client_config` instead. You cannot provide both.
:::
